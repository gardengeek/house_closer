class Lead < ActiveRecord::Base
  ...
  def assign_agent
    if !self.agent
      self.agent =  User.find(:first, :conditions => [ 'users.id = roles_users.user_id and roles_users.role_id = roles.id and roles.title = ? and users.id not IN (select distinct(agent_id) from leads where leads.active = ?)', 'agent', true], :include => [:roles])
      if !self.agent 
